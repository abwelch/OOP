...
struct piece {
  piece() : x(), y(), col(), row(), kind(), match(), alpha(255) {}
...
struct bejeweled_board {
  ...
  friend std::ostream &operator<<(std::ostream &os, bejeweled_board const &b) {
    for (int i = 1; i <= 8; ++i) {
      for (int j = 1; j <= 8; ++j) {
        os << b[i][j].kind << ' ';
      }
...
struct bejeweled_app : event_listener {
  bejeweled_app()
      : prng(std::random_device()()),
        window(VideoMode(740, 480), "Match-3 Game!"), grid(prng) {
...
struct debug_listener : event_listener {
  debug_listener(bejeweled_app &a) : app(&a) {}

  void on_close() override { std::cerr << "closing...\n"; }
